    ...
    <div class="row">
        <div class="col-sm-12">
            @if(Auth::user()->account->links->contains(\App\Link::where('title','=','تعديل مواطن')->first()->id))
                <form style="float:right;display: inline;margin-left: -111px" method="post" enctype="multipart/form-data" action="/account/project/importcitizentoproject/{{$item->id}}">
                {{csrf_field()}}
                <div class="col-sm-6" btn btn-success>
                    <input type="file" name="data_file" />
                </div>
                <div class="col-sm-4">
                    <input type="submit" style="width:70px;" value="رفع" class="btn btn-primary"/>
                </div>
