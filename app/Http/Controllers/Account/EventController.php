...
class EventController extends Controller
{
    public function index(Request $request){
        ...
        {
            $items = $items->where([['start_date', '>=', Carbon::parse($request['start_date'])->format('Y-m-d')], ['start_date', '<=', Carbon::parse($request['end_date'])->format('Y-m-d')]]);

        }

        if (!is_null($request['end_date']))
        {
            $items = $items->where([['end_date', '>=', Carbon::parse($request['start_date'])->format('Y-m-d')], ['start_date', '<=', Carbon::parse($request['end_date'])->format('Y-m-d')]]);
        ...
        if ($request['theaction'] == 'search'){
            ...
            $items = Event::orderBy("id")->paginate(5);
        }
    ...
    public function addEvent(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'event_name.required' => 'طبيعة الإجازة غير محددة، يرجى تحديدها بالشكل الصحيح',
            'start_date.required' => 'التاريخ غير محدد، يرجى تحديده بالشكل الصحيح',
            'end_date.required' => 'التاريخ غير محدد، يرجى تحديده بالشكل الصحيح',
            'end_date.after_or_equal' => 'تاريخ نهاية الإجازة متعارض مع تاريخ البداية، يرجى إدخال التاريخ بالشكل الصحيح',
        ]);
