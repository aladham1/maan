...
class AccountController extends BaseController
{
    public function index(Request $request)
    {
        ...
        // if ($rate_id) {
        //     $accountsids = AccountProjects::where('project_id', "!=", "1")->where('rate', "=", $rate_id)->pluck('account_id');
        //     $items->whereIn("id"
        //         , $accountsids);
        // }
        ...
        if ($request['theaction'] == 'search'){
            $items = $items->orderBy("full_name")->paginate(12)->appends([
                "q" => $q, "circles" => $circles,"full_name" => $account_id , "project_id" => $project_id, "the_type" => $the_type, "rate_id" => $rate_id]);
    ...
    public function store(AccountRequest $request)
    {
        ...
        $isExists = User::where("id_number", $request["id_number"])->count();
        if ($isExists) {

            Session::flash("msg", "e:اسم الموظف مدرج مسبقا في النظام ");
            return redirect("/account/account/create")->withInput();
        }

        // $isExists = User::where("email", $request["email"])->count();
        // if ($isExists) {
        //     Session::flash("msg", "e:البريد الإلكتروني خطأ، يرجى إدخال بريد إلكتروني صحيح وفعال.");
        //     return redirect("/account/account/create")->withInput();
        // }
    
        $isExists = Account::where("user_name", $request["user_name"])->count();
        if ($isExists) {
            Session::flash("msg", "e:اسم المستخدم موجود مسبقا , يرجى إدخال اسم مستخدم مختلف ");
            return redirect("/account/account/create")->withInput();
        }
        // $isExists = Account::where("mobile", $request["mobile"])->count();
        // if ($isExists) {
        //     Session::flash("msg", "e:رقم المحمول خطأ، يرجى إدخال رقم محمول صحيح وفعال.");
        //     return redirect("/account/account/create")->withInput();
        // }
...
//dd($request->all());
    ...
    public function update(AccountRequest $request, $id)
    {
        ...
        // if ($isExists) {
        //     Session::flash("msg", "e:اسم المستخدم موجود مسبقا , يرجى إدخال اسم مستخدم مختلف ");
        //     return redirect("/account/account/ed")->withInput();
        // }
        $item = Account::find($id);
        if ($item == NULL) {
            Session::flash("msg", "e:الرجاء التاكد من الرابط المطلوب");
...
/**********تعديل الإسم بالمشاريع***********/
    ...
    public function formtoaccount($id, Request $request)
    {
        ...
        elseif ($request['theaction'] == 'print') {
            ...
            $items = Form::whereIn('id',$items->pluck('id'))->paginate(20);
