...
class AppendixController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'appendix_file' => 'required|mimes:pdf'
        ],[
            'appendix_name.required' => 'اسم الملحق غير مدخل، يرجى إدخال الاسم بالشكل الصحيح',
            'appendix_describe.required' => 'وصف الملحق غير مدخل، يرجى إدخال الوصف بالشكل الصحيح',
            'appendix_file.required' => 'لم يتم رفع الملحق، يرجى رفع الملحق بالشكل الصحيح',
            'appendix_file.mimes' => 'يجب ان يكون الملحق من نوع pdf',
        ]);
        ...
        if($request->hasFile('appendix_file')){
            ...
            $appendix->original_name=$request->appendix_file->getClientOriginalName();;
    ...
    public function showfiles($id)
    {
        $item = Appendix::find($id);
        if ($item){
            $filename = "uploads/appendix/".$item->appendix_file;
            $path = public_path($filename);

            return Response::make(file_get_contents($path), 200, [
                'Content-Type' => 'application/pdf',
                'Title' => $item->original_name,
                'Content-Disposition' => 'inline; filename="'.$item->original_name.'"'
            ]);
