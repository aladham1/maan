    ...
    public function index(Request $request)
    {
        ...
        if($request->has('private')){
            if( $request['private']== 1){
                $items->where("private", "=", 1);
            }else if( $request['private']== 2){
                $items->where("private", "=", 0);
            }
        }
        ...
        if ($request['theaction'] == 'search') {
            ...
            // $items = "";
             $items = Account::orderBy("full_name")->paginate(5);
        }
...
//        $validate = $this->validate($request, $request->rules());
    ...
    public function profileup(ProfileRequest $request, $id)
    {
        ...
        $isExistsEmail = Account::where("email", $request["email"])->where("id", "!=", $id)->count();

        if ($isExistsEmail > 0)
        {
            Session::flash("msg", "e:البريد الإلكتروني موجود مسبقاً");
            return redirect("/account/account/$id/edit");
        }
        $isExistsUser = Account::where("user_name", $request["user_name"])->where("id", "!=", $id)->count();
        if ($isExistsUser > 0)
        {
            Session::flash("msg", "e:اسم المستخدم موجود مسبقاً");
            return redirect("/account/account/$id/edit");
        }
...
//        $this->validate($request,$request->rules());
    ...
    public function forminaccount($id, Request $request)
    {
        ...
        $items = $item->forms()->whereIn('project_id', Account::find(auth()->user()->account->id)->projects()->pluck('projects.id'))
            ...
            ->join('categories', 'categories.id', '=', 'forms.category_id')
            ->join('citizens', 'citizens.id', '=', 'forms.citizen_id')
            ->rightjoin('form_follows', 'form_follows.form_id', '=', 'forms.id')
            ->rightjoin('form_responses', 'form_responses.form_id', '=', 'forms.id')
            ->select('forms.id',
                'citizens.first_name', 'citizens.father_name', 'citizens.grandfather_name', 'citizens.last_name', 'citizens.id_number'
                , 'categories.name as nammes', 'forms.title',
                'projects.name as zammes', 'project_status.name as pstaname',
        ...
        $items = $items->where(function ($query) {
            ...
            return $query->when(request('status'), function ($query) {
                ...
                if (request('status') != 1) {
                    return $query->whereIn('forms.id', $intersect);
                } else {
                    return $query->whereNotIn('forms.id', $intersect);
                }
        ...
        if ($request['theaction'] == 'excel') {
            ...
            return view("account.account.forminaccount", compact("items", "form_type", "form_status", "sent_typee", "item", "projects", "type", "categories", "project_status"));
        }
    }
