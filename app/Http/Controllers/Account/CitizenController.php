...
class CitizenController extends BaseController
{
    ...
    public function download_citizen_file()
    {
        ...
        $name = "Annex Template 05-" . date('dmY') . ".xlsx";
//        return response()->download($file);
    ...
    public function save_citizen_data(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            [
                'data_file.required' => 'ملف المستفيدين غير محدد، يرجي تحديده',
                'project_id.required' => '"اسم المشروع غير محدد، يرجي تحديده',
            ]);
        ...
        if ($validator->fails()) {
            ...
            if ($request->file('data_file')->isValid()) {
                $import = new CitizensImport($request['project_id']);
                Excel::import($import, request()->file('data_file'));
                if($import->data){
                    $items = $import->data;
                    return Excel::download(new ProblemExport($items), "Annex Template" . date('dmY') . ".xlsx");
                }else{
                    \Session::flash('success', 'تمت عملية إضافة المستفيدين بنجاح.');
                    return Redirect::to("/account/citizen/create");
                }
            }
        }
    }
    ...
    public function not_benefit(Request $request)
    {
        ...
        if ($request['theaction'] == 'excel') {
            ...
            return Excel::download(new CitizenNotbenfitExport($items), "Annex Template 04-" . date('dmY') . ".xlsx");
        } else {
    ...
    public function accept($id)
    {
        ...
        if ($item == NULL ||
            !check_permission('تعديل بيانات المواطن')
...
//                    ->first()->id))
    ...
    public function index(Request $request)
    {
        if (auth()->user()->account->id == 1) {
            $project_ids = Project::pluck('projects.id');
        } else {
            $project_ids = Account::find(auth()->user()->account->id)->projects()->pluck('projects.id');
        ...
        $items = CitizenProjects::whereIn('citizen_id', $citizen_ids)
            ->join('citizens', 'citizen_project.citizen_id', '=', 'citizens.id')
            ->join('projects', 'citizen_project.project_id', '=', 'projects.id')
            ->select('citizens.id',
                'first_name',
                'father_name',
                'grandfather_name',
        ...
        if (request('id')) {
//            $ids = CitizenProjects::where('project_request', request('id'))->pluck('citizen_id');
            $items->where('citizen_project.project_request', request('id'));
        }
        if (request('first_name')) {
            $items->where("first_name", request('first_name'));
        }
        ...
        if (request('project')) {
            $items = $items->where("citizen_project.project_id", request('project'));
        }
        ...
        if ($request['theaction'] == 'excel') {
            $items = $items->orderBy("id", 'desc')->get();
            return Excel::download(new CitizenExport($items), "Annex Template 03-" . date('dmY') . ".xlsx");
    ...
    public function store(CitizenRequest $request)
    {
        ...
        if (request('projects')) {
            \DB::table("citizen_project")->where("citizen_id", $theid)->delete();
            foreach ($request['projects'] as $key => $project_id) {
                $arr = explode("_", $project_id, 2);
                $k = substr($project_id, strpos($project_id, "_") + 1);

                if (empty($request["project_request"][$k])) {
                    Session::flash("msg", "e: رقم طلب المشروع غير مدخل، يرجى إدخال الرقم بالشكل الصحيح");
                    return redirect("/account/citizen/create");

                } else {
                    \DB::table("citizen_project")->insert([
                        "citizen_id" => $theid,
                        "project_id" => $arr[0],
                        "project_request" => $request["project_request"][$k]
                    ]);
                }
            }
        }
    ...
    {
        $project_request = CitizenProjects::find($id);
        $item = Citizen::find($project_request->citizen_id);
        if ($project_request  == NULL &&  $item == NULL) {
            Session::flash("msg", "e:الرجاء التاكد من الرابط المطلوب");
            return redirect("/account/citizen");
        }
        $project_request->delete();
        Session::flash("msg", "تم حذف المستفيد بنجاح");
        return redirect("/account/citizen");
    }

    public function destroy_not_benefit($id)
    {
        $item = Citizen::find($id);
        if ($item == NULL) {
            Session::flash("msg", "e:الرجاء التاكد من الرابط المطلوب");
        ...
        if ($item->forms->toArray() != null) {
            Session::flash("msg", "e:لا يمكن حذف المواطن في حال كان مسجل باسمه اقتراح/شكوى على النظام");
            return redirect("/account/citizen");
        } else {
            $item = Citizen::find($id);
//            if ($item->forms->first()) {
...
//            }
            ...
            return redirect("/account/notbenfit");
        }
...
//        if (Auth::user()->account->links->contains(\App\Link::where('title', '=', 'تعديل مواطن')->first()->id)) {
    ...
    public function selectprojectPost(CitizenRequest $request, $id)
    {
        ...
        \DB::table("citizen_project")->where("citizen_id", $id)->delete();
        if ($request["projects"]) {
            foreach ($request["projects"] as $key => $link) {
                $arr = explode("_", $link, 2);
                $k = substr($link, strpos($link, "_") + 1);
                if (!$request["project_request"][$k]) {
                    Session::flash("msg", "e: رقم طلب المشروع غير مدخل، يرجى إدخال الرقم بالشكل الصحيح");
                    return redirect("/account/citizen/create");
    ...
    public function formincitizen($id, Request $request)
    {
        ...
        if ($request['theaction'] == 'excel') {
            ...
            return Excel::download(new CitizenFormExport($items), "Annex Template 01-" . date('dmY') . ".xlsx");
    ...
    public function showprojects($id)
    {
        ...
        return view("account.citizen.citizenprojects", compact('item'));
